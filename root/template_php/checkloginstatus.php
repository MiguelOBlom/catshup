...
function evalAdmin($dbcon, $id, $u, $p){
    $sql = "SELECT * FROM user WHERE id='$id' AND username='$u' AND password='$p' AND activated='1' AND userlevel='d' LIMIT 1";
    $query = mysqli_query($dbcon, $sql);
    $numrows = mysqli_num_rows($query);
    if($numrows > 0){
        return true;
...
if (isset($_SESSION["userid"]) && isset($_SESSION["username"]) && isset($_SESSION["password"])){
    ...
    if($user_ok){
        $admin = evalAdmin($db_connection,$log_id,$log_username,$log_password);
    }
